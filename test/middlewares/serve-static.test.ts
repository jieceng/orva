...
test('serveStatic serves files from disk with content-type and etag', async () => {
  const app = createOrva().use(serveStatic({ root: FIXTURE_ROOT }));
...
test('serveStatic supports prefix, directory index, and manifest assets', async () => {
  ...
  assert.equal(await indexResponse.text(), '<h1>orva static</h1>\n');
...
test('serveStatic supports spaFallback and function cacheControl', async () => {
  const app = createOrva().use(serveStatic({
    root: FIXTURE_ROOT,
    prefix: '/app',
  ...
  assert.equal(await spaResponse.text(), '<h1>orva static</h1>\n');
